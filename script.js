...
var gridX = 3; //  initial ball grid position x
var gridY = 3; // initial ball grid position y
...
function paddingLoc(pad) {
    ...
    var p = [padSide, padLoc];
    return p;
}

function paddingFunction(pad) {
...
// function checkWall, checks if the ball is next to the wall.  Returns true or false.
function checkWall() {
...
// function addObstacle, parameters; x, y, vX, vY.  Adds a Obstacle(); class to the obs[] array.
...
// function newObs, parameters; location of the new obstacle, and the obstacles velocity.  Uses paddingLoc(); to get the location to create the new obstacle, -
// and then uses addObstacle(); to make the obstacle.
...
// function drawObs, uses a for loop to go through all of the obstacles in obs[] and draws each obstacle.
function drawObs() {
...
// function clearCanvas, clears the canvas based on canvas.hieght and .width
function clearCanvas() {
    ctx.clearRect(0, 0, canvas.width, canvas.height);
}

// function collisionCheck, prints "!" in the console when the ball is hit with an obtacle.
...
// function randomNumber, returns a random number between [1, 25].
...
// functions to set up the canvas


// function drawCheckeredBackground, draws the checkered background on the canvas.
function drawCheckeredBackground(can, nRow, nCol) {

    var w = canvas.width - (canvasPad);
    var h = canvas.height - (canvasPad);
